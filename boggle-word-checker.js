...
const checkAdjacent = (board, target, i, j) => {
  if (target.length === 0) return true;
    ...
    else if (i > 0 && board[i - 1][j] === target[0]) checkAdjacent(board, target.slice(1), i - 1, j);
    else if ((i > 0 && j < board[i].length - 1) && board[i - 1][j + 1] === target[0]) checkAdjacent(board, target.slice(1), i - 1, j + 1);
    else if (j > 0 && board[i][j - 1] === target[0]) checkAdjacent(board, target.slice(1), i, j - 1);
    else if (j < board[i].length - 1 && board[i][j + 1] === target[0]) checkAdjacent(board, target.slice(1), i, j + 1);
    else if (i < board.length - 1 && board[i + 1][j - 1] === target[0]) checkAdjacent(board, target.slice(1), i + 1, j - 1);
    else if (i < board.length - 1 && board[i + 1][j] === target[0]) checkAdjacent(board, target.slice(1), i + 1, j);
    else if ((i < board.length - 1 && j < board[i].length - 1) && board[i + 1][j + 1] === target[0]) checkAdjacent(board, target.slice(1), i + 1, j + 1);
  }
}
